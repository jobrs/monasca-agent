...
|-----------|----------------|
| name | Name of the check instance configuration, useful to read logs etc.|
| dimensions | dimensions will be added to any metrics generated by the check for that instance |

*Beware:* Dimensions are similar to primary keys of a relational database. If you change the set of used dimensions for a metric, existing
visualizations (Grafana dashboards) might stop showing data.
...
|-----------|-------------|---------|
| java_bin_path | points to the _Java_ executable if not in system path | system path |
...
| Attribute | Description | Default |
|-----------|-------------|---------|
| host | hostname of the JMX endpoint | localhost |
| port | port of the JMX endpoint | (mandatory) |
| conf | instance-specific mapping configuration | see init_config |

### JMX Metric Mapping Configurations

Mapping configurations describe how JMX _attributes_ are mapped to Monasca _metrics_. Each
check configuration instance requires a mapping configuration. If it is not defined on
instance-level, it needs to be available in the `init_config` section.

The most important aspect of the mapping is the selection of metrics. This is done via
at least one `include` section and optional `exclude` sections which describe selection
...
```
- include:
    # include attributes of JXM beans with type = Broker
    type: Broker
    # include named list of attributes
    attribute:
      ...
      StorePercentUsage:
        ...
        metric_type: gauge
- include:
    # include attribute by name without adjusting the type
    attribute:
      - MemoryPercentUsage
- exclude:
    # exclude attributes by name
    attribute:
      - TempPercentUsage
```

### Testing JMX Check Configurations

The `jmx` command of the `monasca-agent` can be used to test-drive your JMX configuration.

It offers the following commands:
* list_everything: List all supported attributes/metrics
* list_collected_attributes: List attributes that will actually be collected by your current instances configuration
* list_matching_attributes: List attributes that match at least one of your instance configurations
* list_not_matching_attributes: List attributes that don't match any of your instance configurations
